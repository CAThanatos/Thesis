...
\section{Introduction}
  ...
  \subsection{The Control of Collective Robots}
    ...
    Among all these architectures to control a single robot, the behaviour-based is the one used in most of the MRS~\cite{Mataric2008}.


    -> En gros parler de social deliberation et compagnie.
    -> Dire qu'on fait essentiellement du behaviour-based (Mataric2008)
    -> Puis parler de ce qui se fait en swarm (c'est aussi du behaviour-based)
    -> Terminer par des exemples (tirés de mataric2008) dont RoboCup
    -> Finir en précisant que là c'est plein de cas où les gens écrivent les behaviours à la main

    -> Citer Mataric1995 pour le fait de faire du behaviour-based en MRS




    -> Clairement va falloir commencer par parler de délibération VS behaviour-based. Après on pourra dire comment c'est fait plus exactement
    On peut en parler pour un robot puis dire comment ça se passe pour plusieurs robots (comme dans Iocchi2001)

    -> Peut-être préciser qu'on s'intéresse surtout au cas adaptif ? C'est quand même souvent le cas. En gros dire que parce que c'est de la coop et de la coord de surcroit on veut pouvoir être dynamique
    -> Au passage je peux aussi choisir de ne pas trop m'emmerder, au moins pour le moment. En gros description des approches en single et multi comme prévu
    -> Puis montrer pourquoi en multi le deliberative c'est chaud
    -> Expliquer un peu ce qu'est dur aussi dans le behaviour-based
    -> Citer quelques travaux qui ne font pas de learning juste pour montrer que ça se fait. Pas besoin d'en mettre une chiée. Genre 5 c'est bieng. Un lecteur comprend bien vite l'intérêt de faire du learning et ça sera expliqué après
