...
\section{Introduction}
  ...
  \subsection{Learning to Cooperate}
    ...
    In the previous subsections, we presented the various architectures, both individual and global, that could be used to control a MRS. However, in all of the examples we presented, they lack the ability to truly adapt to an unknown environment. In particular, the assumption was generally made that the MRS would perform under tighly known conditions. If this was not the case, 


    -> Single robot learning
    -> Multi-Agents learning
    -> Multi-Robots learning

    En gros ya un problème d'explosion du nombre d'états en team learning et un problème de continuous states en robotique ?

    -> Dire TRES rapidement les différents types de learning: supervised, non-supervised and reward-based (je sais plus qui disait qu'il fallait différencier reward-based et reinforcement)
    -> Brambilla2012 pour le RL aussi (pour les swarms)
    ...
    -> Commencer aussi par dire que le truc de Nagpal c'est impressionant mais pas très adaptable
